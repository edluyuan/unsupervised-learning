...
class GibbsSampler:
    ...
    def update_topic_doc_words(self):
        ...
        """
        # Compute the probability distribution over topics for each word in each document
        # Shape: [n_docs, n_topics, n_words]
        # Avoid division by zero by adding a small epsilon
        epsilon = 1e-12
        topic_probs = self.theta[:, :, np.newaxis] * self.phi[:, np.newaxis, :]
        topic_probs /= (self.theta @ self.phi + epsilon)
    ...
    def update_loglike(self, iteration):
        ...
        ll += np.sum(gammaln(self.alpha * self.n_topics) - self.n_topics * gammaln(self.alpha))
        ll += np.sum(gammaln(self.alpha + self.A_dk) - gammaln(np.sum(self.alpha + self.A_dk, axis=1)))

        # Topic-Word distributions
        ll += np.sum(gammaln(self.beta * self.n_words) - self.n_words * gammaln(self.beta))
        ll += np.sum(gammaln(self.beta + self.B_kw) - gammaln(np.sum(self.beta + self.B_kw, axis=1)))
        ...
        if self.do_test:
            ...
            # Document-Topic distributions
            ll_test += np.sum(gammaln(self.alpha + self.A_dk_test) - gammaln(np.sum(self.alpha + self.A_dk_test, axis=1)))

            # Topic-Word distributions
            ll_test += np.sum(gammaln(self.beta + self.B_kw_test) - gammaln(np.sum(self.beta + self.B_kw_test, axis=1)))
